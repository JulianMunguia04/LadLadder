...
io.on("connection", (socket) => {
  ...
  socket.on("reconnect-player", async (savedPlayerId, roomCode)=>{
    ...
    if(player){
      ...
      try{
        const questionEntry = currentRoom.questions[currentRoom.question];

        const currentQuestionId = questionEntry?._id ? questionEntry._id : questionEntry;

        let currentQuestion = null;
        if (currentQuestionId) {
          currentQuestion = await Questions.findById(currentQuestionId);
        }
    ...
    try {
      ...
      if (updatedRoom.questions.length < updatedRoom.players.length * 2) {
        socket.to(roomCode).emit(
          "player-question",
          (updatedRoom.questions.length / 2),
        ...
        // ✅ KEEP GAME IN PROMPTING PHASE
        updatedRoom.currentGameState = "prompt";
        await updatedRoom.save();

      } else {
        // ✅ NOW switch to ranking ONLY when ready
        updatedRoom.currentGameState = "ranking";
        await updatedRoom.save();

        const firstEntry = updatedRoom.questions[0];
        const firstQuestionId = firstEntry._id ? firstEntry._id : firstEntry;

        const firstQuestion = await Questions.findById(firstQuestionId);
        const players = await Players.find({
          _id: { $in: updatedRoom.players }
        }).select('_id name playerNumber');
  ...
  socket.on("ranked-answer-submit", async (roomCode, playerId, rankedPlayers) => {
    try {
      const updatedRoom = await Room.findOneAndUpdate(
        { 
          room: roomCode,
          "currentAnswers.playerId": { $ne: playerId },
          resultsShown: { $ne: true }
        },
        { 
          $push: { currentAnswers: { playerId, rankedPlayers } } 
        },
        { new: true }
      );

    if (!updatedRoom) {
      // means duplicate submit or room missing
      return;
    }
      ...
      const answerCount = updatedRoom.currentAnswers.length;
      const totalPlayers = updatedRoom.players.length;

      socket.to(roomCode).emit("ranked-answer-submitted", answerCount, totalPlayers);
      socket.emit("ranked-answer-submitted", answerCount, totalPlayers);
  
      // Check if all players have submitted their answers
      if (answerCount === totalPlayers) {

        const result = await Room.updateOne(
          { room: roomCode, resultsShown: { $ne: true } },
          { $set: { resultsShown: true } }
        );

        // Only ONE socket will succeed here
        if (result.modifiedCount === 0) {
          return; // another socket already handled results
        }
        ...
        let questionNumber = updatedRoom.question;
        const questionEntry = updatedRoom.questions[questionNumber];
        const questionId = questionEntry._id ? questionEntry._id : questionEntry;
  ...
  socket.on("next-question", async (roomCode)=>{
    try {
      ...
      currentRoom.resultsShown = false; 
      ...
      if (currentRoom.questions[currentRoom.question] !== undefined){
        ...
        if (!questionEntry) {
          console.log("No question found at index:", currentRoom.question);
          return;
        }

        // Handles BOTH ObjectId and populated object
        const currentQuestionId = questionEntry._id ? questionEntry._id : questionEntry;
...
async function processAnswers(roomCode) {
  try {
    ...
    const currentAnswersAsStrings = currentAnswers.map((entry) =>
      entry.rankedPlayers.map((id) => id.toString())
    );
